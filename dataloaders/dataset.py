...
class VideoDataset(Dataset):
    ...
    def preprocess(self):
        ...
        for file in os.listdir(self.root_dir):
            ...
            # train_and_valid, test = train_test_split(video_files, train_size=0.8, random_state=42)
            video_files = shuffle(video_files, random_state=42)

            train_split = int(len(video_files) * .8)
            val_split = int(len(video_files) * .1) + train_split
            # test_split = int(len(video_files) * .1) + val_split + train_split
            train, val, test = video_files[:train_split], video_files[train_split: val_split], video_files[val_split:]
            # train, val = train_test_split(train_and_valid, train_size=0.8, random_state=42)
